...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // On successful sign-up, redirect to the sign-in page.
  ...
  return (
    <div className="relative flex items-center justify-center min-h-screen overflow-hidden bg-slate-950">
      ...
      <div className="relative z-10 w-full max-w-md p-8 space-y-8 bg-slate-900/60 backdrop-blur-sm border border-slate-700 rounded-2xl shadow-2xl">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-white tracking-tight">Create an Account</h1>
          <p className="mt-2 text-slate-400">Start your creative journey today</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="relative">
            <UserPlus className="absolute w-5 h-5 top-3 left-3 text-slate-400" />
            <input
              id="name"
              type="text"
              value={name}
        ...
        <p className="text-sm text-center text-slate-400">
          ...
          <Link href="/signin" className="font-medium text-cyan-400 hover:underline">
            Sign in
          </Link>
        </p>
      </div>
